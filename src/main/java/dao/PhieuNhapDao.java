...
public class PhieuNhapDao {
	...
	/**
	 * Tạo phiếu nhập mới, lưu chi tiết hàng loạt và cập nhật Lô hàng trong cùng một
	 * giao dịch (Transaction)
	 * 
	 * @return ID (MaPhieuNhap) nếu thành công, trả về null nếu có lỗi và tự
	 *         rollback.
	 */
	public Integer savePhieuNhapTransaction(int maNguoiDung, Integer maNCC, String ghiChu,
		...
		try (Connection con = ConnectDB.getCon()) {
			...
			try {
				...
				try (PreparedStatement psPN = con.prepareStatement(sqlPhieuNhap, Statement.RETURN_GENERATED_KEYS)) {
					...
					if (maNCC != null)
						...
						psPN.setNull(2, java.sql.Types.INTEGER);
					if (ghiChu != null)
						psPN.setNString(3, ghiChu);
					else
						psPN.setNull(3, java.sql.Types.NVARCHAR);
				...
				// Vòng lặp Chi tiết nhập kho
				// *** SQL ĐỐI SOÁT: INSERT vào ChiTietPhieuNhap chỉ 6 cột, KHÔNG ghi ThanhTien
				// (PERSISTED) ***
				// Bỏ tiền tố dbo. để tránh lỗi schema mismatch nêú user login bằng role không
				// thuộc dbo
				...
				try (PreparedStatement psCT = con.prepareStatement(sqlChiTiet);
					...
					for (int i = 0; i < chiTietList.size(); i++) {
						...
						System.out.println("[Transaction DAO] Xử lý Dòng " + i + ": Sản phẩm ID " + ct.getMaSanPham()
								+ ", Lô " + ct.getSoLo());
						...
						if (ct.isMergeBatch()) {
							...
							if (maNCC != null)
								psUpdLo.setInt(5, maNCC);
							else
								psUpdLo.setNull(5, java.sql.Types.INTEGER);
							...
							System.out.println("[Transaction DAO] -> CẬP NHẬT CỘNG DỒN Lô hàng cũ: " + ct.getSoLo()
									+ " (HSD: " + ct.getHanSuDung() + ")");
						} else {
							// Lô mới -> INSERT
							String loaiHB = ct.getLoaiHinhBan() != null ? ct.getLoaiHinhBan() : "Bán lẻ";
							...
							if (maNCC != null)
								psInsLo.setInt(3, maNCC);
							else
								psInsLo.setNull(3, java.sql.Types.INTEGER);
					...
					try (PreparedStatement psTongTien = con
							.prepareStatement("UPDATE dbo.PhieuNhap SET TongTien = ? WHERE MaPhieuNhap = ?")) {
			...
			// Connection creation or setAutoCommit failed
			e.printStackTrace();
			return null;
